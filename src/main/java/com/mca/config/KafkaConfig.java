...
public class KafkaConfig implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		try {
			...
			stocks.forEach(stock -> kafkaMessageProducer.sendMessage("VideoGameEvent",gson.toJson(stock).toString()));
		} catch (Exception e) {
	...
	public VideoGameEvent convertStock(List<String> stock) {
		return VideoGameEvent.builder()
				.videoGameId(Integer.parseInt(stock.get(0)))
				.availability(Boolean.parseBoolean(stock.get(1)))
				.updateTime(
					Timestamp.valueOf(LocalDateTime.parse(stock.get(2), DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSSX")))
				)
				.build();
	}
}
